...
class MailThread(models.AbstractModel):
    ...
    def message_route(
        ...
        email_from = message_dict["email_from"]
        email_to_list = [
            e.lower() for e in (tools.email_split(message_dict["to"]) or [""])
        ]
        ...
        rcpt_tos_list = [
            e.lower() for e in (tools.email_split(message_dict["recipients"]) or [""])
        ]
        ...
        for model, thread_id, custom_values, user_id, alias in routes or ():
            ...
            if (
                ...
                thread_root = thread_root.with_context(
                    from_alias=True, mail_create_nosubscribe=not partner_from_found
                )
    ...
    def _message_parse_extract_payload(
        self, message, message_dict, save_original=False
    ):
        ...
        if message.get_content_maintype() == "text":
            ...
            for part in message.walk():
                ...
                if part.get_content_type() == "text/xml" and not part.get_param(
                    "charset"
                ):
    ...
    def _get_bounced_message_data(self, message, message_dict):
        ...
        reference_ids.extend(
            tools.mail_header_msgid_re.findall(message_dict["in_reply_to"])
        )
        reference_ids.extend(
            tools.mail_header_msgid_re.findall(message_dict["references"])
        )
    ...
    def _get_parent_message(self, msg_dict):
        ...
        reference_ids = tools.mail_header_msgid_re.findall(
            msg_dict.get("references") or ""
        )
        if reference_ids:
            parent = self.env["mail.message"].search(
    ...
    def _message_add_suggested_recipient(
        ...
        if partner and partner.email:  # complete profile: id, name <email>
            ...
            result[self.ids[0]].append(
                (partner.id, partner.name or "", lang, reason, {})
            )
        else:  # unknown partner, we are probably managing an email address
            _, parsed_email_normalized = parse_contact_from_email(email)
            partner_create_values = self._get_customer_information().get(
                parsed_email_normalized, {}
            )
            result[self.ids[0]].append(
                (
                    False,
                    partner_info.get("full_name") or email,
                    lang,
                    reason,
    ...
    def _mail_find_user_for_gateway(self, email_value, alias=None):
        ...
        if (
            self.env["mail.alias"]
            .sudo()
            .search_count([("alias_full_name", "=", email_value)])
        ):
            return self.env["res.users"]
        ...
        if not matching_user:
            std_users = (
                self.env["res.users"]
                .sudo()
                .search([("email_normalized", "=", normalized_email)], limit=1)
            )
    ...
    def _mail_find_partner_from_emails(
        ...
        normalized_emails = [
            email_normalized
            for email_normalized in (
                tools.email_normalize(contact, strict=False) for contact in emails
            )
            if email_normalized
        ]
        matching_aliases = (
            self.env["mail.alias"]
            .sudo()
            .search([("alias_full_name", "in", normalized_emails)])
        )
        if matching_aliases:
            normalized_emails = [
                email
                for email in normalized_emails
                if email not in matching_aliases.mapped("alias_full_name")
            ]
    ...
    def message_post(
        ...
        **kwargs,
        ...
        if attachments:
            ...
            format_error = not tools.is_list_of(
                attachments, list
            ) and not tools.is_list_of(attachments, tuple)
            if not format_error:
    ...
    def _process_attachments_for_post(
        ...
        if attachments:  # generate
            ...
            for attachment, (cid, name, token) in zip(
                new_attachments, attachement_extra_list
            ):
                if cid:
    ...
    def _message_log_with_view(
        ...
        """
        self._raise_for_invalid_parameters(
            set(kwargs.keys()), forbidden_names={"body", "bodies"}
        )
        ...
        for lang, lang_recipients_data in lang_to_recipients.items():
            ...
            if not lang_model_description:
                lang_model_description = record_wlang._get_model_description(
                    msg_vals["model"]
                    if msg_vals and msg_vals.get("model")
                    else message.model
                )
    ...
    def _notify_by_email_prepare_rendering_context(
        ...
        if email_add_signature:
            ...
            author_user = (
                ...
                else author.user_ids[0] if author and author.user_ids else False
            )
            if author_user:
        ...
        if check_tracking:
            tracking_values = (
                ...
                .filtered(
                    lambda track: not track.field_groups
                    or self.env.is_superuser()
                    or self.user_has_groups(track.field_groups)
                )
            )
    ...
    def _is_thread_message(self, msg_vals=None):
        ...
        res_id = (
            ...
            else (self.ids[0] if self.ids else False)
        )
    ...
    def _message_update_content(
        ...
        **kwargs,
        ...
        if partner_ids:
            msg_values.update({"partner_ids": list(partner_ids or [])})
        if msg_values:
        ...
        self.env["bus.bus"]._sendone(
            message._bus_notification_target(), "mail.record/insert", payload
        )
    ...
    def _get_mail_thread_data(self, request_list):
        ...
        if "activities" in request_list:
            res["activities"] = self.with_context(
                active_test=True
            ).activity_ids.activity_format()
    ...
    def _extract_partner_ids_for_notifications(
        self, message, msg_vals, recipients_data
    ):
        notif_pids = []
        no_inbox_pids = []
    ...
    def _truncate_payload(self, payload):
        ...
        if payload_length > 4096:
            ...
            payload["options"]["body"] = body.encode()[:body_max_length].decode(
                errors="ignore"
            )
        return payload
    ...
    def _notify_thread_by_web_push(
        ...
        partner_ids = self._extract_partner_ids_for_notifications(
            message, msg_vals, recipients_data
        )
        if not partner_ids:
            return
    ...
    def _generate_tracking_message(self, message, return_line="\n"):
        ...
        for value in message.sudo().tracking_value_ids.filtered(
            ...
            if value.field_id.ttype == "boolean":
                ...
                old_value = (
                    value.old_value_char
                    if value.old_value_char
                    else str(value.old_value_integer)
                )
                new_value = (
                    value.new_value_char
                    if value.new_value_char
                    else str(value.new_value_integer)
                )
