...
class ChannelController(http.Controller):
    @http.route(
        "/discuss/channel/members", methods=["POST"], type="json", auth="public"
    )
    ...
    @http.route(
        "/discuss/channel/messages", methods=["POST"], type="json", auth="public"
    )
    @add_guest_to_context
    def discuss_channel_messages(
        ...
        channel = (
            request.env["discuss.channel"].search([("id", "=", channel_id)])
        )  # --source code change-- 加了sudo()
        if not channel:
            raise NotFound()
        domain = [
            ("res_id", "=", channel_id),
        ...
        _logger.info(request.env.user._is_public())
        _logger.info(around)
    ...
    @http.route(
        "/discuss/channel/pinned_messages", methods=["POST"], type="json", auth="public"
    )
    @add_guest_to_context
    def discuss_channel_pins(self, channel_id):
        ...
        if not channel:
            raise NotFound()
        return channel.pinned_message_ids.sorted(
            key="pinned_at", reverse=True
        ).message_format()
    ...
    def discuss_channel_mute(self, channel_id, minutes):
        ...
        if minutes == -1:
            ...
            request.env.ref("mail.ir_cron_discuss_channel_member_unmute")._trigger(
        ...
        request.env["bus.bus"]._sendone(
            member.partner_id, "mail.record/insert", {"Thread": channel_data}
        )

    @http.route(
        "/discuss/channel/update_custom_notifications",
        methods=["POST"],
        type="json",
        auth="user",
    )
    def discuss_channel_update_custom_notifications(
        self, channel_id, custom_notifications
    ):
        channel = request.env["discuss.channel"].search([("id", "=", channel_id)])
        if not channel:
            raise request.not_found()
    ...
    @add_guest_to_context
    def discuss_channel_mark_as_seen(
        self, channel_id, last_message_id, allow_older=False
    ):
        channel = request.env["discuss.channel"].search([("id", "=", channel_id)])
        if not channel:
            raise NotFound()
    ...
    @http.route(
        "/discuss/channel/notify_typing", methods=["POST"], type="json", auth="public"
    )
    @add_guest_to_context
    ...
    @http.route(
        "/discuss/channel/attachments", methods=["POST"], type="json", auth="public"
    )
    @add_guest_to_context
