...
class ThreadController(http.Controller):
    ...
    def mail_message_post(self, thread_model, thread_id, post_data, context=None):
        ...
        )._check_attachments_access(post_data.get("attachment_tokens"))
        if context:
            request.update_context(**context)
        canned_response_ids = tuple(
            cid
            for cid in post_data.pop("canned_response_ids", [])
        ...
        if "body" in post_data:
            post_data["body"] = Markup(
                post_data["body"]
            )  # contains HTML such as @mentions
        new_partners = []
        if "partner_emails" in post_data:
            new_partners = [
                record.id
                for record in request.env["res.partner"]._find_or_create_from_emails(
                    post_data["partner_emails"],
                    post_data.get("partner_additional_values", {}),
                )
            ]
        post_data["partner_ids"] = list(
            set((post_data.get("partner_ids", [])) + new_partners)
        )
    ...
    def mail_message_update_content(
        ...
        guest.env[message_sudo.model].browse(
            [message_sudo.res_id]
